...
Num bs_price_ad(OptionType type, const Num& S, const Num& K, const Num& r, const Num& sigma,
    ...
    const Num d1 = (log(S / K) + (r + 0.5 * sigma * sigma) * T) / (sigma * sqrtT);
    const Num d2 = d1 - sigma * sqrtT;
    const Num disc = exp((-1.0) * r * T);
    if (type == OptionType::Call)
        return S * norm_cdf_ad(d1) - K * disc * norm_cdf_ad(d2);
    return K * disc * norm_cdf_ad((-1.0) * d2) - S * norm_cdf_ad((-1.0) * d1);
}
...
inline Greeks black_scholes_greeks_ad(OptionType type, double S, double K, double r, double sigma,
                                      double T) {
    // First order: seed S, sigma, r, T (K is constant) in one Dual<4> evaluation.
    using D = Dual<4, double>;
    const D price = bs_price_ad<D>(type, ad_var<4>(S, 0), ad_const<4>(K), ad_var<4>(r, 2),
                                   ad_var<4>(sigma, 1), ad_var<4>(T, 3));
    Greeks g{};
    g.price = price.v;
    g.delta = price.d[0];
    g.vega = price.d[1];
    ...
    const DD pr = bs_price_ad<DD>(type, Snn, dd_const(K), dd_const(r), dd_const(sigma), dd_const(T));
    g.gamma = pr.d[0].d[0];
    return g;
}
