...
// Volatility such that black_scholes_price(type, S, K, r, sigma, T) == price.
inline double implied_vol(OptionType type, double price, double S, double K, double r, double T,
    ...
    for (int i = 0; i < max_iter; ++i) {
        const double p = black_scholes_price(type, S, K, r, sigma, T);
        ...
        const double vega = S * norm_pdf(d1) * sqrtT;
